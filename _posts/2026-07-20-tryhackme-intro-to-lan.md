...
**Here's where my first analysis was wrong, and it's worth showing the mistake.** In my notes I reasoned that the only possible bad actor is the *new device Z*, and since Z can only accept or reject what it's offered, there's little room to misbehave. That's true — *about the client*. But it's looking at the wrong actor. The real DHCP attacks come from impersonating the **server**:

- **Rogue DHCP server.** Nothing in the handshake verifies that a DHCP server is *legitimate*. An attacker puts their own DHCP server on the network and races to answer Discover messages first. Their malicious Offer can set the **default gateway to the attacker's own machine** — so now the victim sends all its outbound traffic through the attacker (a man-in-the-middle), or hand over a malicious DNS server to redirect the victim's lookups.
- **DHCP starvation.** An attacker floods the server with Discover requests using fake MAC addresses until the pool of available IPs is exhausted, so legitimate devices can't get an address — a denial of service, often used to knock out the real server and clear the way for a rogue one.
