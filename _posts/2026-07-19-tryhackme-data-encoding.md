...
- **Input validation and normalisation.** This is where encoding stops being about display and becomes a way to attack a website. To follow it, keep three jobs separate in your mind: **validation** (a website checking whether incoming text is safe), **encoding** (writing a character in an alternative form), and **decoding** (turning it back). The attack works by making those three happen in the wrong order. Here's the whole thing as a sequence.

  *The setup.* Imagine an online form on a website — say a box where a customer types the name of a file they want to download. Whatever the customer submits gets sent to the website's server, which then goes and fetches that file from storage.

  *The danger the website is trying to block.* Web servers treat the characters `../` as an instruction: "go up one folder." So if an attacker types `../../../` into that form instead of a normal filename, the server can be tricked into climbing *out* of the intended folder and reaching sensitive files elsewhere on the system. (This attack has a name: *path traversal*.)

  *The website's defence — validation.* To stop this, the website adds a filter that inspects everything submitted through the form and **rejects any input containing `../`**. This checking step is the validation. On the face of it, it works: type `../../../` and the form refuses it.

  *The attacker's move — encoding.* The attacker knows the `/` character can *also* be written in an alternative form. On the web, `/` can be encoded as `%2f` (this is ordinary URL-encoding, the same mechanism that turns a space into `%20`). So instead of `../`, the attacker submits `..%2f..%2f..%2f` into the form.

  *Why the defence fails — validation happens before decoding.* The filter inspects the submitted text, looks for the literal string `../`, and doesn't find it — it only sees `..%2f`, which looks harmless. So it lets the input through. *Then*, further along, the server **decodes** `%2f` back into `/` before using it — and now the dangerous `../../../` exists, past the filter, right as the server acts on it. The filter validated one version of the string; the server acted on a different one. Same characters, submitted in a form the filter wasn't looking for.

  *The fix — normalisation, i.e. decode first, then validate.* The correct order is to **decode and convert the input into one single, standard form first, and only then run the filter** — so the filter checks the exact same string the server will actually use. Converting everything to one canonical form before checking it is called **normalisation**. The rule of thumb is "normalise, *then* validate," never the other way round. It's the same "same bytes, different reading" idea from earlier in this post — except here the mismatch isn't an accident that produces gibberish, it's a deliberate trick that slips an attack past a filter.
